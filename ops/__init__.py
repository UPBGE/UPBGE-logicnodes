...
class NLRemoveTreeByNameOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for ob in [x for x in context.scene.objects if x.select_get() and tools.object_has_treeitem_for_treename(x, self.tree_name)]:
            gs = ob.game
...
class NLApplyLogicOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        selected_objects = [ob for ob in current_scene.objects if ob.select_get()]
...
class NLGenerateLogicNetworkOperator(bpy.types.Operator):
    ...
    def poll(cls, context):
        if not context.space_data:
            print('TREE TO EDIT NOT FOUND! ABORTING...')
            return
        tree = context.space_data.edit_tree
        if not tree: return False
        if not (tree.bl_idname == bge_netlogic.ui.BGELogicTree.bl_idname): return False
...
class NLSwitchInitialNetworkStatusOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        scene = context.scene
        updated_objects = [ob for ob in scene.objects if ob.select_get() and bge_netlogic.utilities.object_has_treeitem_for_treename(ob, tree_name)]
