...
class ParameterObjectProperty(ParameterCell):
    ...
    def evaluate(self):
        ...
        if none_or_invalid(game_object) or (not property_name):
            ...
            self._set_value((game_object.get(property_name, property_default))*10)
...
#class ActionFindChildByName(ActionCell):
#	def __init__(self):
#		ActionCell.__init__(self)
#		self.game_object = None
#		self.game_object_child_name = None
#		self.game_object_child = None
#		self.game_object_parent = None
#		self.CHILD = LogicNetworkSubCell(self, self.get_child)
#		
#	def get_child(self):
#		game_object = self.get_parameter_value(self.game_object)
#		child_name = self.get_parameter_value(self.game_object_child_name)
#		self._set_value(_name_query(parent.childrenRecursive, query))
#		for x in bge.logic.getCurrentScene().objects:
#			if x.parent.name == game_object.type and x.name == child_name:
#				return x
#			else:
#				return
#		return
...
#			return
#		if not none_or_invalid(game_object):
#			return
#	pass
...
class ActionMouseLook(ActionCell):
	...
	def evaluate(self):
		...
		if condition:
			...
			mouse = bge.logic.mouse
			mouse_position = mathutils.Vector(mouse.position)
...
class ActionTrackTo(ActionCell):
	...
	def evaluate(self):
		...
		if moving_object is STATUS_WAITING: return
		if target_object is STATUS_WAITING: return
		if speed is STATUS_WAITING: return
		...
		if rot_axis == 0:
			self._set_value(xrot_to(moving_object, target_object, front_axis, speed, self.network.time_per_frame))
		elif rot_axis == 1:
			self._set_value(yrot_to(moving_object, target_object, front_axis, speed, self.network.time_per_frame))
		elif rot_axis == 2:
			self._set_value(zrot_to(moving_object, target_object, front_axis, speed, self.network.time_per_frame))
